...
DECLARE_EVENT_CLASS(sched_llc_template,
	...
	TP_printk("dst_util=%lu dst_cap=%lu src_util=%lu src_cap=%lu",
		  __entry->dst_util, __entry->dst_cap, __entry->src_util,
		  __entry->src_cap)
);

DEFINE_EVENT(sched_llc_template, sched_llc_nofit,
	     TP_PROTO(unsigned long dst_util, unsigned long dst_cap,
		     unsigned long src_util, unsigned long src_cap),
	     TP_ARGS(dst_util, dst_cap, src_util, src_cap));

DEFINE_EVENT(sched_llc_template, sched_llc_to_forb,
	     TP_PROTO(unsigned long dst_util, unsigned long dst_cap,
		     unsigned long src_util, unsigned long src_cap),
	     TP_ARGS(dst_util, dst_cap, src_util, src_cap));

DEFINE_EVENT(sched_llc_template, sched_llc_out_forb,
	     TP_PROTO(unsigned long dst_util, unsigned long dst_cap,
		     unsigned long src_util, unsigned long src_cap),
	     TP_ARGS(dst_util, dst_cap, src_util, src_cap));
