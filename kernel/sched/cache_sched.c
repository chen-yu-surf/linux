...
/*
 * The per-group footprint is credited to whichever group the task belongs to
 * at NUMA-hinting-fault time (task_numa_placement()) and debited from the
 * group it belongs to at exit (exit_mm_sched_cache()).  When prctl() moves a
 * task between groups, move its footprint estimate too: otherwise the group it
 * leaves over-estimates its footprint forever, and the group it joins
 * under-estimates it once the task exits.  Best-effort and lockless, matching
 * the credit/debit path it mirrors (min()-clamped so the unsigned counter
 * cannot wrap).
 */
...
/*
 * Swap a task's cache group pointer and return the previous one, which the
 * caller owns and must put.  Callers must hold p->pi_lock: it serializes all
 * writers - prctl(PR_SCHED_CACHE), exec_mmap() and exit_mm() - so the old
 * reference is dropped exactly once.
 *
 * Lock order: p->pi_lock nests inside tasklist_lock (read-locked by the prctl
 * PIDTYPE_TGID/PGID loop) and inside task_lock()/->alloc_lock (held across
 * exec_mmap()).  It is a leaf lock here and nothing below it sleeps.
 */
struct sched_cache_group *
