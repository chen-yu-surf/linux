...
void sched_cache_exec_mmap(struct task_struct *tsk, struct mm_struct *mm)
{
	struct sched_cache_group *old;

	/*
	 * Acquire the new reference before publishing the pointer, then drop
	 * the old one. @tsk is current and the only writer of its own pointer.
	 */
	old = rcu_replace_pointer(tsk->sched_cache_grp,
...
void sched_cache_exit_mm(struct task_struct *p)
{
	struct sched_cache_group *grp =
		rcu_replace_pointer(p->sched_cache_grp, NULL, p == current);
