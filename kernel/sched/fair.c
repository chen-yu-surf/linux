		  ...
		  struct sched_cache_time __percpu *_pcpu_sched)
{
	...
	/*
	 * Publish the group last.  Every reader qualifies it by
...
static void set_delayed(struct sched_entity *se)
{
	 ...
	 * This check can be removed because with flat pick stuff
	 * only task can get delayed, same for clear_delayed().
	 * 
	 */
	if (!entity_is_task(se)) {
		se->sched_delayed = 1;
