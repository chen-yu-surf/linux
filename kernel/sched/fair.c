...
enqueue_task_fair(struct rq *rq, struct task_struct *p, int flags)
{
	...
	if ((flags & ENQUEUE_WAKEUP) && last_dequeue &&
		...
		/*
		 * The enqueue task's CPU has already been assigned
		 * with a new one. Need to calculate the task sleeping
...
dequeue_throttle:
	...
	if (task_sleep) {
		...
		/* if this rq is about to become idle, update its cache hot timeout */
		if (sched_feat(SIS_CACHE) && !rq->nr_running &&
		    p->avg_hot_dur)
			rq->cache_hot_timeout = max(rq->cache_hot_timeout, now + p->avg_hot_dur);
	} else {
		/*
		 * HACK, 0 indicates the dequeue is not caused by sleep, used by
		 * enqueue_task_fair().
		 */
...
/* Return true if the idle CPU is cache-hot, return false otherwise. */
