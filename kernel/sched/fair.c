...
static void task_cache_work(struct callback_head *work)
{
	...
	/*
	 * Make sure grp is not released by others.
	 * The rcu lock can not be hold util
	 * zalloc_cpumask_var() below, because
	 * the latter might sleep.
	 */
	scoped_guard (rcu)
		grp = sched_cache_group_get(rcu_dereference(p->sched_cache_grp));
	if (!grp)
		return;
	...
	scoped_guard (cpus_read_lock) {
		...
		for_each_cpu(cpu, cpus) {
			...
			for_each_cpu(i, sched_domain_span(sd)) {
				...
				/*
				 * Count the tasks sharing our cache group, not
				 * our mm: the group is the unit being placed.
