				 ...
				 int id)
{
	 ...
	 * race due to out-of-range access to llc_counts:
	 *
	 * CPU0                                CPU1
	 * :
	 * ...
...
{
	int cpu = p->mm->sc_stat.cpu, curr_cpu = task_cpu(p);
				...
				struct sched_group *group)
{
	 ...
	 */
	sd_share = rcu_dereference_all(per_cpu(sd_llc_shared,
