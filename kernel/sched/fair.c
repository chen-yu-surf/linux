...
enqueue_task_fair(struct rq *rq, struct task_struct *p, int flags)
{
	 ...
	 * Use wakee's previous CPU's clock to get the delta.
	 * The previous CPU should be online, otherwise the
	 * sleep time could be larger than expected(the clock
	...
	if ((flags & ENQUEUE_WAKEUP) && last_dequeue && cpu_online(task_cpu(p)) &&
	    now > last_dequeue)
		update_avg(&p->se.sleep_avg, now - last_dequeue);
