...
On a machine with several last level caches (LLCs) -  it can pay off to
keep a set of cooperating tasks on CPUs that share one LLC, so that the
data they pass between each other stays cache hot instead of bouncing
