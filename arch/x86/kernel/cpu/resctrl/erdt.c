...
#define MMRC_VALID_INDEX_FUNC_VERSION 1
#define MARC_VALID_INDEX_FUNC_VERSION 1
...
static void __iomem *marc_index_function_1(struct erdt_domain_info *d,
					   int type, int closid_idx)
{
	return d->base[type] + closid_idx * 8;
}
			     ...
			     int closid_idx, int region_offset_bits)
{
	...
	vaddr = marc_index_function_1(d, type, closid_idx);
...
static bool mmrc_init(struct erdt_domain_info *d, struct acpi_subtbl_hdr_16 *subtbl)
{
	...
	if (mmrc->index_fn != MMRC_VALID_INDEX_FUNC_VERSION) {
...
static bool marc_init(struct erdt_domain_info *d, struct acpi_subtbl_hdr_16 *subtbl)
{
	...
	if (marc->index_fn != MARC_VALID_INDEX_FUNC_VERSION) {
