...
/*
 * A single global value erdt_scale is used mainly because of the limbo handler:
 * resctrl_rmid_realloc_threshold is a single global value and
 * resctrl_arch_round_mon_val() takes no domain argument, so one scale has to
 * be derived from the per-domain cmrc->up_scale. max() was chosen because the
 * rounding is a floor: a larger scale yields a slightly lower threshold, i.e.
 * an RMID has to drop to a slightly lower occupancy before it is reused.
 */
...
__init bool erdt_get_mem_config(struct rdt_resource *r)
{
	...
	legacy_ctrl = erdt_legacy_mba_ctrl(r);
	if (!legacy_ctrl)
		return false;
	...
	for (region = 0; region < max_regions; region++) {
		for (type = RESCTRL_CTRL_REGION_TYPE_OPT; type < RESCTRL_CTRL_REGION_NR_CTRLS; type++) {
			...
			hw_ctrl->r_ctrl.scalar.max = marc->mba_ctrl_range;
			hw_ctrl->r_ctrl.scalar.min = 1;
			hw_ctrl->r_ctrl.scalar.gran = 1;
			/*
			 * A MIN control defaults to the smallest guarantee, the
			 * OPT and MAX controls to the full range.
			 */
			if (type == RESCTRL_CTRL_REGION_TYPE_MIN)
				hw_ctrl->r_ctrl.scalar.reset_val = hw_ctrl->r_ctrl.scalar.min;
			else
				hw_ctrl->r_ctrl.scalar.reset_val = hw_ctrl->r_ctrl.scalar.max;
